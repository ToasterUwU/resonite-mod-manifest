...
def check_for_updates(info: dict):
    ...
    try:
        ...
        print(f"Failed to fetch releases for {info['name']}: {e}")  # Failed to fetch releases
        return info
    ...
    for release in releases:
        ...
        base_version = re.match(r'^(\d+(?:\.\d+)*)(?:-?(rc|b|beta|alpha|preview|dev|test|canary|exp|experimental)[\.-]?\d*)?$', stripped_tag, re.IGNORECASE)
        if base_version:
        ...
        if len(version_parts) < 3:
            normalized_tag = '.'.join(version_parts + ['0'] * (3 - len(version_parts)))  # Pad to 3 digits
        else:
            normalized_tag = '.'.join(version_parts)

        is_prerelease = bool(re.search(r'-(rc|b|beta|alpha|preview|dev|test|canary|exp|experimental)', stripped_tag, re.IGNORECASE))
        ...
        previous_versions_sorted = sorted(previous_versions, key=lambda v: [int(x) if x.isdigit() else x for x in v.split('.')], reverse=True)  # Sort previous versions
            ...
            for prev_artifact in info["versions"][prev_version].get("artifacts", []):
                ...
                if prev_name:
                    if prev_name not in previous_artifact_info:
                        previous_artifact_info[prev_name] = {k: v for k, v in prev_artifact.items() if k not in ("url", "sha256")}  # Copy special args from previous artifact
        ...
        for asset in asset_list:
            ...
            try:
                ...
                print(f"Failed to download asset {asset_url}: {e}")  # Failed to download asset
                continue
            ...
            if prev_info:
                artifact_entry.update(prev_info)  # Inherit previous special args if present
            else:
                modname = info.get("name", "").lower()
                is_library = info.get("category", "").lower() == "libraries"
                if asset_name.lower().endswith(".dll") and modname not in asset_name.lower():
                    artifact_entry["installLocation"] = "/rml_libs"  # DLL dependency logic
                if is_library:
                    artifact_entry["installLocation"] = "/rml_libs"  # Libraries category
                config_exts = (".json", ".toml", ".yaml", ".yml", ".ini", ".conf", ".cfg", ".txt", ".xml", ".properties", ".env", ".config")
                if asset_name.lower().endswith(config_exts):
                    artifact_entry["installLocation"] = "/rml_config"  # Config file logic
    ...
    if "versions" in info and info["versions"]:
        ...
        info["versions"] = dict(sorted(info["versions"].items(), key=lambda x: version_key(x[0]), reverse=True))  # Sort versions
